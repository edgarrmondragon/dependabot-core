...
module Dependabot
  module Python
    module Shared
      # Base class for Python requirement parsing, shared between Python and UV ecosystems.
      # Subclasses can override methods to add ecosystem-specific behavior.
      class PythonRequirementParser
        ...
        def user_specified_requirements
          base_requirements.compact
        end

        # Subclasses override this to add additional requirement sources
        sig { overridable.returns(T::Array[T.nilable(String)]) }
        ...
        def pip_compile_python_requirement
          requirement_files.each do |file|
            next unless compiled_file?(file)
        ...
        # Subclasses must implement this to check if a file is a compiled pip-compile output
        sig { abstract.params(file: Dependabot::DependencyFile).returns(T::Boolean) }
        def compiled_file?(file); end
        ...
        # Subclasses must implement this to return the requirement class
        sig { abstract.returns(T.class_of(Dependabot::Requirement)) }
